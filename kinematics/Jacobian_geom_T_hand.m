...
function J = Jacobian_geom_T_hand(w_T_all, index_base, index_finger, index_link, i_p_point)
...
assert(length(i_p_point) == 3, '[Jacobian_geom_T_sharedbase]: input index_base is incorrect! ')
J = zeros(6,nl);
w_p_point = T_p31(w_T_all(:,:,index_link),reshape(i_p_point,3,1));
% w_T_all(:,:,index_link)*[reshape(i_p_point,3,1);1];
