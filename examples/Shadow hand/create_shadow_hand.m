...
finger_middle = Finger('Middle', 'type','R_RRRR', 'l_links',finger_dimension_r); 
finger_ring = Finger('Ring', 'type','R_RRRR', 'l_links',finger_dimension_r); 
...
finger_middle.set_mdh_parameters(mdh_matrix);
finger_ring.set_mdh_parameters(mdh_matrix);
...
finger_middle.set_base_p(R*[0.133;0.011;0]); 
finger_middle.set_base_R(R*euler2R_XYZ([0,0,0]));
finger_ring.set_base_p(R*[0.129;-0.011;0]); 
finger_ring.set_base_R(R*euler2R_XYZ([0,0,0]));
...
finger_middle.update_finger(q_I_0);
finger_ring.update_finger(q_I_0);
...
% set middle finger
finger_middle.list_links(1).set_mass(0); % in kg
finger_middle.list_links(1).set_inertia([0,0,0,0,0,0]); 

finger_middle.list_links(2).set_mass(0.03); % in kg
finger_middle.list_links(2).set_inertia([1.8100e-06,1.0000e-05,9.8100e-06,0,0,0]); 

finger_middle.list_links(3).set_mass(0.017); % in kg
finger_middle.list_links(3).set_inertia([8.6842e-07,2.7257e-06,2.6761e-06,0,0,0]); 

finger_middle.list_links(4).set_mass(0.012); % in kg
finger_middle.list_links(4).set_inertia([5.3425e-07,1.0530e-06,9.3925e-07,0,0,0]);
finger_middle.update_finger_par_dyn;

% set ring finger
finger_ring.list_links(1).set_mass(0); % in kg
finger_ring.list_links(1).set_inertia([0,0,0,0,0,0]); 

finger_ring.list_links(2).set_mass(0.03); % in kg
finger_ring.list_links(2).set_inertia([1.8100e-06,1.0000e-05,9.8100e-06,0,0,0]); 

finger_ring.list_links(3).set_mass(0.017); % in kg
finger_ring.list_links(3).set_inertia([8.6842e-07,2.7257e-06,2.6761e-06,0,0,0]); 

finger_ring.list_links(4).set_mass(0.012); % in kg
finger_ring.list_links(4).set_inertia([5.3425e-07,1.0530e-06,9.3925e-07,0,0,0]);
finger_ring.update_finger_par_dyn;
...
%% add tendons (fully actuated version)

if finger_index.nt == 0
    finger_index.add_tendon('Flex_1', [zeros(3,4);1,1,1,1]);
    finger_index.add_tendon('Flex_2', [zeros(3,4);-1,1,1,0]);
    finger_index.add_tendon('Flex_3', [zeros(3,4);1,1,0,0]);
    finger_index.add_tendon('Flex_4', [zeros(3,4);-1,0,0,0]);
    finger_index.add_tendon('Ext_1', [zeros(3,4);1,-1,-1,-1]);
    finger_index.add_tendon('Ext_2', [zeros(3,4);-1,-1,-1,0]);
    finger_index.add_tendon('Ext_3', [zeros(3,4);1,-1,0,0]);
    finger_index.add_tendon('Ext_4', [zeros(3,4);-1,0,0,0]);
end
...
if finger_middle.nt == 0
    finger_middle.add_tendon('Flex_1', [zeros(3,4);1,1,1,1]);
    finger_middle.add_tendon('Flex_2', [zeros(3,4);-1,1,1,0]);
    finger_middle.add_tendon('Flex_3', [zeros(3,4);1,1,0,0]);
    finger_middle.add_tendon('Flex_4', [zeros(3,4);-1,0,0,0]);
    finger_middle.add_tendon('Ext_1', [zeros(3,4);1,-1,-1,-1]);
    finger_middle.add_tendon('Ext_2', [zeros(3,4);-1,-1,-1,0]);
    finger_middle.add_tendon('Ext_3', [zeros(3,4);1,-1,0,0]);
    finger_middle.add_tendon('Ext_4', [zeros(3,4);-1,0,0,0]);
end

finger_middle.update_finger(q_I_0);
finger_middle.update_M_coupling(q_I_0);
finger_middle.M_coupling;

if finger_ring.nt == 0
    finger_ring.add_tendon('Flex_1', [zeros(3,4);1,1,1,1]);
    finger_ring.add_tendon('Flex_2', [zeros(3,4);-1,1,1,0]);
    finger_ring.add_tendon('Flex_3', [zeros(3,4);1,1,0,0]);
    finger_ring.add_tendon('Flex_4', [zeros(3,4);-1,0,0,0]);
    finger_ring.add_tendon('Ext_1', [zeros(3,4);1,-1,-1,-1]);
    finger_ring.add_tendon('Ext_2', [zeros(3,4);-1,-1,-1,0]);
    finger_ring.add_tendon('Ext_3', [zeros(3,4);1,-1,0,0]);
    finger_ring.add_tendon('Ext_4', [zeros(3,4);-1,0,0,0]);
end
finger_ring.update_finger(q_I_0);
finger_ring.update_M_coupling(q_I_0);
finger_ring.M_coupling;

if finger_thumb.nt == 0
    finger_thumb.add_tendon('Flex_1', [zeros(3,5);1,1,-1,1,1]);
    finger_thumb.add_tendon('Flex_2', [zeros(3,5);-1,1,1,1,0]);
    finger_thumb.add_tendon('Flex_3', [zeros(3,5);1,1,-1,0,0]);
    finger_thumb.add_tendon('Flex_4', [zeros(3,5);-1,1,0,0,0]);
    finger_thumb.add_tendon('Flex_5', [zeros(3,5);1,0,0,0,0]);
    finger_thumb.add_tendon('Ext_1', [zeros(3,5);1,-1,-1,-1,-1]);
    finger_thumb.add_tendon('Ext_2', [zeros(3,5);-1,-1,1,-1,0]);
    finger_thumb.add_tendon('Ext_3', [zeros(3,5);1,-1,-1,0,0]);
    finger_thumb.add_tendon('Ext_4', [zeros(3,5);-1,-1,0,0,0]);
    finger_thumb.add_tendon('Ext_5', [zeros(3,5);1,0,0,0,0]);
end
finger_thumb.update_finger(q_T_0);
...
if finger_little.nt == 0
    finger_little.add_tendon('Flex_1', [zeros(3,5);1,1,-1,1,1]);
    finger_little.add_tendon('Flex_2', [zeros(3,5);-1,1,1,1,0]);
    finger_little.add_tendon('Flex_3', [zeros(3,5);1,1,-1,0,0]);
    finger_little.add_tendon('Flex_4', [zeros(3,5);-1,1,0,0,0]);
    finger_little.add_tendon('Flex_5', [zeros(3,5);1,0,0,0,0]);
    finger_little.add_tendon('Ext_1', [zeros(3,5);1,-1,-1,-1,-1]);
    finger_little.add_tendon('Ext_2', [zeros(3,5);-1,-1,1,-1,0]);
    finger_little.add_tendon('Ext_3', [zeros(3,5);1,-1,-1,0,0]);
    finger_little.add_tendon('Ext_4', [zeros(3,5);-1,-1,0,0,0]);
    finger_little.add_tendon('Ext_5', [zeros(3,5);1,0,0,0,0]);
end
finger_little.update_finger(q_L_0);
...
finger_middle.list_joints(1).q_limits = [-20,20]*pi/180; % joints
finger_middle.list_joints(2).q_limits = [-15,90]*pi/180; % joints
finger_middle.list_joints(3).q_limits = [0,90]*pi/180; % 
finger_middle.list_joints(4).q_limits = [0,90]*pi/180; % 
finger_middle.update_joints_info;

finger_ring.list_joints(1).q_limits = [-20,20]*pi/180; % joints
finger_ring.list_joints(2).q_limits = [-15,90]*pi/180; % joints
finger_ring.list_joints(3).q_limits = [0,90]*pi/180; % 
finger_ring.list_joints(4).q_limits = [0,90]*pi/180; % 
finger_ring.update_joints_info;
...
shadow_hand = Hand('Shadow Hand');

shadow_hand.add_finger(finger_thumb);
shadow_hand.add_finger(finger_index);
shadow_hand.add_finger(finger_middle);
shadow_hand.add_finger(finger_ring);
shadow_hand.add_finger(finger_little);

plot_par = shadow_hand.plot_parameter_init;
plot_par.axis_len = 0.05;
plot_par.com_markersize = 20;

shadow_hand.update_hand(zeros(shadow_hand.nj,1))
...
shadow_hand.plot_hand(plot_par)
shadow_hand.plot_hand_com(plot_par)
axis equal
