...
% symbolic version according to invdyn_ne_xq_mdh_all_fext.m
% Input:
...
function [FTau,W_T_allframe] = invdyn_ne_xq_mdh_all_fext_sym(xq,xqD,xqDD,mdh,Mass,F_ext,CoM,I,g)
%#codegen
...
for i = n_q+1:-1:1
    ...
    F(1:3,i) = simplify(F(1:3,i+1) - m_i*g + m_i*VD_c(1:3,i) - F_ext(1:3,i));
    F(4:6,i) = simplify(F(4:6,i+1) - cross(-W_i_r_c,F(1:3,i) + F_ext(1:3,i)) - cross(W_c_r_ip1,-F(1:3,i+1)) ...
        + W_I_i*VD_c(4:6,i) + cross(V(4:6,i),W_I_i*V(4:6,i)) - F_ext(4:6,i));
...
for i = 2:n_q+1
    ...
    W_T_i = W_T_allframe(:,:,i);
    W_R_i = W_T_i(1:3,1:3);
    Tau(i-1) = simplify(F(4:6,i)'*W_R_i*[0 0 1]');
end
