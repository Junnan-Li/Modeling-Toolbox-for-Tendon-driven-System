% calculate Gravity vector using Lagrangian Euler method
% Input:
...
function G = invdyn_lag_mdh_G(q, mdh, x_base, Mass, CoM, g) % , X_base, XD_base,XDD_base 
%#codegen
...
for i = 1:n_q
    ...
    w_p_i = w_T_i(1:3,4);
    G_i = 0;
    for j = i:n_q % iteration from 1 to i
        w_T_j = w_T_all(:,4*j-3:4*j);
        w_R_j = w_T_j(1:3,1:3);
        w_p_j = w_T_j(1:3,4);
        w_p_mj = w_p_j + w_R_j * CoM(:,j+1); % pos of com_j in base frame
        Jt_ij = cross(w_R_i*[0;0;1],w_p_mj-w_p_i); % translational J, 3x1
        G_i = G_i - Mass(j+1)*g'*Jt_ij;
    end
    G(i) = G_i;
end
