...
function [qDD,XDD_base,M_fd,C_fd,G_fd] = fordyn_ne_mdh_fb(q,qD,Tau,mdh, Mass, X_base, XD_base, F_ext, CoM, I, g, varargin)
...
% gravity term
if mex
    [G_tau,F_ext] = invdyn_ne_mdh_mex(q,zeros(n_q,1),zeros(n_q,1),mdh, Mass,...
        X_base, zeros(6,1), zeros(6,1), zeros(6,1), CoM, I, g);
else
    [G_tau,F_ext] = invdyn_ne_mdh(q,zeros(n_q,1),zeros(n_q,1),mdh, Mass,...
        X_base, zeros(6,1), zeros(6,1), zeros(6,1), CoM, I, g);
end
G_fd = [F_ext(1,:);G_tau];
...
% C term
if mex
    [C_tau,F_ext] = invdyn_ne_mdh_mex(q,qD,zeros(n_q,1),mdh, Mass,...
        X_base, XD_base, zeros(6,1), zeros(6,1), CoM, I, g);
else
    [C_tau,F_ext] = invdyn_ne_mdh(q,qD,zeros(n_q,1),mdh, Mass,...
             X_base, XD_base, zeros(6,1), zeros(6,1), CoM, I, g);
end
C_fd = [F_ext(1,:);C_tau];
C_fd = C_fd - G_fd;
...
% M term
M_fd = zeros(n_q+6,n_q+6);

for i = 1:n_q+6
    qDD_M = zeros(n_q,1);
    qDD_M(i) = 1;
    state_M = zeros(n_q+6,1);
    state_M(i) = 1;
    if mex
        [M_tau,F_ext] = invdyn_ne_mdh_mex(q,zeros(n_q,1),state_M(7:end),mdh, Mass,...
            X_base, XD_base, state_M(1:6), zeros(6,1), CoM, I, g);
    else
        [M_tau,F_ext] = invdyn_ne_mdh(q,zeros(n_q,1),state_M(7:end),mdh, Mass,...
            X_base, XD_base, state_M(1:6), zeros(6,1), CoM, I, g);
    end
    M_fd(:,i) = [F_ext(1,:);M_tau];
    M_fd(:,i) = M_fd(:,i) - G_fd;
end
...
qDD = (M_fd)\(Tau + tau_ext - G_fd - C_fd);
