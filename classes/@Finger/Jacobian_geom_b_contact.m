% Geometric Jacobian of the contact points based on the jonit cofiguration
% reference coordinate: CS base of the finger
% 
% Output:
%           J: [6xobj.nja] geometric jacobian
% 
% Source: 
%   "Robotics: Modeling, Planning, and Control" P111 - P113
% 
% Note: to be tested and debugged

function J = Jacobian_geom_b_contact(obj,q, contacts)

% if nargin == 1
%     q = obj.q;
% end



assert(length(q)== obj.nja, 'dimension of joint vector is incorrect!')
assert(isequal(class(contacts),'Contacts'), 'Input type is incorrect!')

q = reshape(q,[obj.nja,1]);

num_active_joint = obj.nja;
% mdh_struct = obj.get_finger_mdh(q);
mdh = mdh_struct_to_matrix(obj.mdh,1);

base_T_end = T_mdh_multi(mdh);

for i = 1:num_active_joint
    
    if contacts.link_index+1 >= i

        base_T_i = T_mdh_multi(mdh(1:i,:));
        b_R_i = base_T_i(1:3,1:3);
        b_p_i = base_T_i(1:3,4);
        b_i_p_c = contacts.base_p - b_p_i;
        J_tmp_p = cross(b_R_i*[0;0;1],b_i_p_c);
        J_tmp_w = b_R_i*[0;0;1];
        J(:,i) = [J_tmp_p;J_tmp_w];
    else
        J(:,i) = [0,0,0,0,0,0];
    end
end

end