        ...
        for i = 1:size(muscle_i.constr_obs,1)
            ...
            if size(constr_obs_i,1)==3
                ...
                if wrap_status_i
                    ...
                    w_J_Q = obj.Jacobian_geom_w_point(constr_obs_i{2}.index_link_inhand, T_p31(inv(w_T_link),w_Q_p));
                    w_J_T = obj.Jacobian_geom_w_point(constr_obs_i{2}.index_link_inhand, T_p31(inv(w_T_link),w_T_p));
                    J_vp(:,:,1) = w_J_P;
                    J_vp(:,:,2) = w_J_Q;
                    J_vp(:,:,3) = w_J_T;
                    J_vp(:,:,4) = w_J_S;
