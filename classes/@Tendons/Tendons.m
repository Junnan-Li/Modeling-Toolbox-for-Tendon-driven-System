...
classdef Tendons < handle
    ...
    properties (Access = public)
        ...
        moment_arm           % [njx1] moment arms in m 
    ...
    methods
        function obj = Tendons(name, routing, index)
            ...
            for i = 1:obj.j_index
                obj.par_MA_poly3(:,i) = routing(i).* [0;0;0.1;0.01]; % default value 10mm
            end
        ...
        function init_tendon_par(obj, nj_finger, q)
            ...
            obj.moment_arm = obj.cal_ma_poly3(q);
        ...
        function moment_arm = cal_ma_poly3(obj, q)
            % calculate the moment arm value with q using poly3 expression
            % MA =  ax3 + bx2+ cx + d
            moment_arm = zeros(obj.nj_finger,1);
            for i = 1:obj.j_index
                q_i = q(i);
                ...
                moment_arm(i) = a*q_i^3 + b*q_i^2 + c*q_i + d;
            end
        end
